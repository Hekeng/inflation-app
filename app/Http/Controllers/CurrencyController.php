<?php
// Инструмент,          Что на входе (аргументы),           Что внутри функции (callback),					Что возвращает (выход)
// collect($arr),       Обычный массив,                     —,												Объект-коллекция
// pluck('key'),        Имя ключа (строка),                 —,												Коллекция только из значений этого ключа
// filter(fn),          Анонимная функция,					Элемент массива. Должен вернуть true/false,		"Коллекция только с теми данными, где было true"
// map(fn),             Анонимная функция,					Элемент массива. Должен вернуть новое значение,	Коллекция с измененными значениями
// keys(),              Нет,								—,												"Коллекция, состоящая только из ключей"
// all(),               Нет,								—,												Обычный массив (вынимает данные из коробки)

// В JS строка — это почти объект, у неё есть методы типа .length или .toUpperCase(). В PHP строка — это примитив. Ты не можешь написать $str->length().

// Как работать со строками в Laravel: Laravel дает крутой помощник Str. Это объектный способ работы со строками.

// Длина строки: strlen($str) (чистый PHP) или Str::length($str) (Laravel).

// Поиск и замена: Str::contains($str, 'USA') (вернет true/false).

// Превращение в массив: explode(', ', $str).

// Цикл по строке: Напрямую через foreach строку перебрать нельзя. Чтобы это сделать, её нужно сначала превратить в массив: str_split($str).

// Совет: В Laravel старайся использовать хелпер str($string)->upper()->trim(). Это называется "Fluent interface" (текучий интерфейс), и он очень


// Раз тебе зашел оператор ?? (Null Coalescing), вот еще пара "сахарных" штук:

// Элвис-оператор ?: $result = $a ?: 'default'; — если $a приводится к false (ноль, пустая строка), возьми дефолт.

// Null-safe оператор ?-> (Киллер-фича!) Представь, что ты ищешь город, но страны может не существовать: $city = $country?->city?->name; Если $country — null, код не упадет с ошибкой, а просто вернет null во всю цепочку. В обычном PHP тебе пришлось бы писать три if.

// $list = ["Apple", "Banana"];

// $list[] = "Orange"; // Добавить в конец (как .push() в JS) 🍎
// unset($list[0]);    // Удалить элемент по ключу 🗑️
// $count = count($list); // Узнать количество элементов 📏

// isset($var) — проверяет, существует ли переменная и не равна ли она null.

// empty($var) — проверяет, является ли значение "пустым" (ноль, пустая строка, пустой массив, false или null — всё это для неё "пусто").

namespace App\Http\Controllers;

use Illuminate\Http\Request;

class CurrencyController extends Controller
{
    public function index()
    {
        define("BASE_CURRENCY", "php artisan serve");
        $newCountries = [
            "Ukraine" => [
                "rate" => 1.2,
                "currency" => "UAH",
                "year" => 1995
            ],
            "USA" => [
                "rate" => 2.2,
                "currency" => "USD",
                "year" => 2000
            ],
            "Canada" => [
                "rate" => 2,
                "currency" => "USD",
                "year" => 2010
            ],
            "Germany" => [
                "rate" => 0.5,
                "currency" => "Euro",
                "year" => 2020
            ]
        ];




        $color = [];
        foreach ($newCountries as $country => $value) {
            if ($country !== "USA" && $value['rate'] > 1) {
                $color[] = 'red';
            } else {
                $color []= 'green';
            };
        }

        dump($color);
            return view('welcome', ['result' => $color, 'countries' => $newCountries]);
    }

    public function showCountry($name)
    {
        // Теперь $name равно тому, что юзер ввел в URL (например, "usa")
        return "Вы ищете данные по стране: " . $name;
    }

    public function debug ($slug){
            return "Параметр из URL: " . $slug;
    }
}
// $country = strtoupper(trim(" ukraine "));

// $collection = collect($newCountries) -> keys() -> map(fn($name) => str($name)->lower()) -> all();
// $result = collect($newCountries)
// ->keys() // 1. Получаем ["Ukraine", "USA", "Canada", "Germany"]
// ->map(function($name) { // 2. Берем каждое имя и трансформируем его
//     return str($name)->lower();
// })
// ->all();
// // $collection = keys(collect($newCountries));
// dump($collection);



// $laravelResult = collect($newCountries)->pluck('rate')->map(fn($r) => $r * 100)->all();

// $step1 = collect($newCountries);

// $step_filter = $step1 -> filter(fn($item) => $item['rate']> 1);

// $step2 = $step_filter->keys();

// $step3 = $step2->all();

// BASE_CURRENCY = "USD";
// $inflationRate = 5.5;
// $indexName = "CPI";
// $countryName = "Canada"; // Используем camelCase: countryName вместо country
// $currentRate = 1.2;

// $countries = ["Ukraine", "USA", "Canada"];

// $newCountries = ["Ukraine"=>1.2, "USA"=>2.2, "Canada"=>2, "Germany"=>0.5];



// dd() dump($step3);

// $result = collect($newCountries)
//         ->filter(fn($item) => isset($item['rate']))
//         ->keys() // Тут мы получили массив названий стран ["Ukraine", "USA"...]
//         ->map(fn($name) => strtoupper($name)); // Тут превращаем названия в капс



// $result = collect($newCountries)->filter(fn($item) => $item['rate'] > 1) -> keys();

// $result = collect($newCountries)
//         ->filter(fn($item) => ($item['rate'] ?? 0) > 1)
//         ->map(fn($item) => $item['rate'] * 100)
//         ->keys()
//         ->all();

            // $step_filter = $step1 -> filter (function($item) {
            // // $item — это внутренний массив каждой страны
            // $currentRate = $item['rate'];

            // if ($currentRate > 1) {
            //     return true;  // Оставляем в списке
            // } else {
            //     return false; // Выкидываем
            // }
            // });

            // function newFilter ($arr){

            //     foreach ($arr as $key => $value) {
            //         $arr2 = [];
            //             if ($value == "rate") {
            //                 $arr2 = $value;
            //             }
            //     }
            //         $sortedTrueArr = [];
            //     foreach ($arr2 as $key1 => $value1) {

            //         if ($value1 > 1) {
            //            $sortedTrueArr = $value1;  // Оставляем в списке
            //         }
            //     }
            //     return $sortedTrueArr;
            // }
